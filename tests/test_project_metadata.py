...
def test_pyproject_metadata_and_entry_points():
    ...
    assert [author["name"] for author in project["authors"]] == ["Amine Güneş", "Rafał Kotyński"]
    ...
    assert (root / "examples" / "richardson_lucy_motion_blur.py").exists()
    assert (root / "examples" / "kaczmarz_gaussian.py").exists()
    assert (root / "examples" / "auto_richardson_lucy_motion.py").exists()
...
def test_pdf_source_has_no_named_author_and_contains_ai_disclosure():
    ...
    assert "Automatic parameter selection" in tex
    assert "auto_deconvolve" in tex
