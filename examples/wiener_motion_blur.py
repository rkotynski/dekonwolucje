#!/usr/bin/env python3
"""Standalone Wiener-deconvolution example using the public deconv API.
...
applies the explicit FFT/IFFT Wiener filter, and saves both individual images
...
def main(output_dir: Path | str = "wiener_motion_output") -> Path:
    ...
    result = wiener_filter(disturbed, psf, K=2.0e-3, non_negative=True)
    ...
    save_grayscale(restored, output_path / "restored_wiener.png")

    figure, axes = plt.subplots(1, 4, figsize=(14, 4))
    panels = (
        (reference.data, "Reference"),
        (psf.kernel, "Oblique motion PSF"),
        (disturbed.data, "Disturbed image"),
        (restored.data, "Wiener restoration"),
    )
